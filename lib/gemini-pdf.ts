...
export function getClient(apiKeyOverride?: string | null) {
    const raw =
...
export function getResponseText(result: { response: { text: () => string } }): string {
    try {
...
export async function extractProductsFromPdf(
    ...
    opts?: {
        ...
        /** Estrazione batch: solo un intervallo di pagine del catalogo completo. */
    ...
    const sliceHint = slice
        ? `\n\nCONTESTO BATCH: Il PDF qui allegato contiene SOLO le pagine da ${slice.from} a ${slice.to} del catalogo completo${
              slice.totalPages != null ? ` (${slice.totalPages} pagine totali).` : "."
          } Per ogni prodotto, "pageNumber" deve essere il numero di pagina nel catalogo COMPLETO (quindi tra ${slice.from} e ${slice.to}, coerente con il PDF allegato).\n`
        : "";
