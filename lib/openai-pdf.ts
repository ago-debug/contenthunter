...
export async function openaiExtractProductsFromPdf(
    ...
    opts?: {
        ...
        pageSlice?: { from: number; to: number; totalPages?: number };
    }
): Promise<ExtractResult> {
    const slice = opts?.pageSlice;
    const sliceHint = slice
        ? `CONTESTO BATCH: il PDF è solo le pagine ${slice.from}–${slice.to} del catalogo${
              slice.totalPages != null ? ` (${slice.totalPages} pagine totali).` : "."
          } pageNumber = pagina globale nel catalogo (${slice.from}…${slice.to}).\n\n`
        : "";
    // Per semplicità, passiamo il PDF come base64 nel prompt; per documenti grandi
    // conviene prima estrarre il testo lato nostro.
    const cap = slice ? Math.min(pdfBase64.length, 400_000) : 10_000;
    const user = `${sliceHint}Questo è un PDF codificato in base64. Usa solo i contenuti reali che riesci a interpretare, non inventare nulla.

BASE64_PDF:
${pdfBase64.substring(0, cap)}

${slice ? "Il PDF è uno slice: usa tutto il base64 sopra (troncato se necessario)." : "Se il PDF è troppo lungo, concentrati sulle prime pagine dove compaiono prodotti, listini o tabelle."}`;
