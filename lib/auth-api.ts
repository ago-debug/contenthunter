...
/** Solo header `x-company-id` (sync). Utile nei POST canali dove non si legge la sessione. */
export function getCompanyIdFromHeaders(req: Request): number | null {
    const h = req.headers.get("x-company-id");
    if (!h) return null;
...
/**
 * Anteprime dal browser verso GET /api/product-images (niente header x-company-id):
 * admin globale può leggere; altrimenti il companyId di sessione o da query deve coincidere col prodotto.
 */
export async function canAccessProductImageForCompany(
    req: Request,
    ...
    const fromReq = await getCompanyIdFromRequest(req);
    const companyId = getCompanyId(session, fromReq);
    return companyId != null && companyId === productCompanyId;
}
