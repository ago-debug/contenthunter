...
/**
 * Directory `public` dell’app (upload PDF in `public/uploads/...`).
 * Su alcuni host (Passenger, cwd diverso) impostare `PDF_CATALOG_PUBLIC_ROOT` con path assoluto alla cartella `public`.
 */
function getPublicDir(): string {
    const env = process.env.PDF_CATALOG_PUBLIC_ROOT?.trim();
    if (env) {
...
/**
 * Content-Disposition per inline PDF: `filename="..."` solo ASCII (Node/undici rifiuta header non Latin-1 in alcuni casi).
 * Aggiunge `filename*=UTF-8''…` per il nome originale.
 */
export function buildPdfInlineContentDisposition(originalFileName: string | null | undefined): string {
    const base = (originalFileName || "document.pdf").trim() || "document.pdf";
    const ascii = base.replace(/[^\x20-\x7E]/g, "_").replace(/"/g, "_") || "document.pdf";
