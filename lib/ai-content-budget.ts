/**
 * Limiti condivisi per generazione contenuti prodotto (OpenAI).
...
export const AI_PRODUCT_COPY_FAST = {
    docDescriptionMaxChars: 520,
    brandGuidelinesMaxChars: 260,
    extraFieldsMaxChars: 260,
} as const;
...
export function maxOutputTokensProductCopy(requestedBlockCount: number, fastMode: boolean): number {
    ...
    if (fastMode) {
        const cap = parseCap(process.env.OPENAI_PRODUCT_COPY_MAX_TOKENS_FAST, 240);
        const per = 78;
        const min = 70;
        return Math.min(cap, Math.max(min, per * n));
    }
    const cap = parseCap(process.env.OPENAI_PRODUCT_COPY_MAX_TOKENS_FULL, 420);
    const per = 125;
    const min = 130;
    return Math.min(cap, Math.max(min, per * n));
}
