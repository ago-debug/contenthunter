...
export default function PdfHub() {
    ...
    const handleExtract = async (pdfId: number) => {
        if (!selectedId) return;
        const batchSize = 6;
        ...
        try {
            const meta = await axios.get<{ pageCount?: number }>(
                "/api/repositories/" + selectedId + "/pdfs/" + pdfId + "/page-count"
            );
            const totalPages = typeof meta.data?.pageCount === "number" ? meta.data.pageCount : 0;
            const run = (payload: Record<string, unknown>) =>
                axios.post("/api/repositories/" + selectedId + "/pdfs/" + pdfId + "/extract", payload, {
                    timeout: 300_000,
                });
            ...
            if (totalPages <= batchSize || totalPages <= 0) {
                ...
                for (let i = 0; i < nB; i++) {
                    ...
                    sum += res.data?.count ?? 0;
                    if (i < nB - 1) await new Promise((r) => setTimeout(r, 450));
                }
            }
            toast.update(toastId, {
                render: "Estratti " + sum + " prodotti." + (totalPages > batchSize ? ` (${totalPages} pag.)` : ""),
                type: "success",
                isLoading: false,
                autoClose: 3000,
            });
            const r = await axios.get("/api/catalogues/" + selectedId);
            setCatalog(r.data);
        } catch (err: any) {
            const msg =
                err?.response?.data?.error ||
    ...
    const handleDelete = async (pdfId: number, fileName: string) => {
        if (!selectedId) return;
        if (!(await appConfirm("Eliminare \"" + fileName + "\" dal catalogo?"))) return;
        axios
