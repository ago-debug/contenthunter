...
interface TableManagerProps {
    title: string;
    /** Base URL per POST/PUT/DELETE (es. `/api/categories`). */
    endpoint: string;
    /** URL per il GET della tabella se diverso (es. `/api/categories?all=true`). */
    listEndpoint?: string;
    fields: {
        key: string;
        label: string;
        type: "text" | "number" | "select" | "textarea" | "searchableSelect";
        options?: { value: string | number; label: string }[];
        /** Opzioni dinamiche (es. escludi la riga in modifica e i figli). */
        optionBuilder?: (ctx: { rows: any[]; editingItem: any | null }) => { value: string | number; label: string }[];
        searchablePlaceholder?: string;
        searchableSearchPlaceholder?: string;
        required?: boolean;
        /** Rendering celle tabella (ricerca usa ancora i valori grezzi della riga). */
        formatCell?: (ctx: { value: unknown; row: any; rows: any[] }) => ReactNode;
    }[];
}
...
export default function TableManager({ title, endpoint, listEndpoint, fields }: TableManagerProps) {
    ...
    const fetchData = async () => {
        ...
        setLoading(true);
        try {
            const resp = await axios.get(listEndpoint ?? endpoint, reqConfig);
            setData(resp.data);
        } catch (err) {
            console.error(err);
    ...
    useEffect(() => {
        void fetchData();
    }, [endpoint, listEndpoint, effectiveCompanyId, sessionStatus, reqConfig]);
    ...
    const handleDelete = async (id: number) => {
        if (!(await appConfirm("Sei sicuro di voler eliminare questo elemento?"))) return;
        try {
    ...
    const handleBulkDelete = async () => {
        ...
        if (!(await appConfirm(`Eliminare ${selectedIds.length} elementi selezionati?`))) return;
                            ...
                            filteredData.map((item) => (
                                <tr key={item.id} className={`hover:bg-slate-50/50 transition-all group ${selectedIds.includes(item.id) ? "bg-slate-50/80" : ""}`}>
                                    ...
                                    {fields.map((f) => {
                                        ...
                                        const custom = f.formatCell?.({ value: raw, row: item, rows: data });
                                        const cell =
                                            f.formatCell != null
                                                ? custom
                                                : f.type === "textarea" && raw
                                                  ? String(raw).slice(0, 60) + (String(raw).length > 60 ? "…" : "")
                                                  : (raw ?? "—");
                                        return (
                                            <td
                                                key={f.key}
                                                className="px-6 py-4 text-sm font-bold text-slate-600 max-w-[min(420px,40vw)]"
                ...
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
                    <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
                        ...
                        <form onSubmit={handleSave} className="p-8 space-y-6">
                            {fields.map((f) => {
                                const selectOptions =
                                    f.type === "searchableSelect"
                                        ? f.optionBuilder
                                            ? f.optionBuilder({ rows: data, editingItem })
                                            : f.options ?? []
                                        : [];
                                const selectValue =
                                    f.type === "searchableSelect"
                                        ? formData[f.key] === null || formData[f.key] === undefined || formData[f.key] === ""
                                            ? null
                                            : Number(formData[f.key])
                                        : null;
                                return (
                                <div key={f.key} className="space-y-2">
                                    ...
                                    {f.type === "textarea" ? (
                                        ...
                                        <SearchableSelect
                                            ...
                                            value={Number.isNaN(selectValue as number) ? null : selectValue}
                                            onChange={(v) =>
                                                setFormData({
                                                    ...formData,
                                                    [f.key]: v === null || v === undefined ? null : Number(v),
                                                })
                                            }
                                            placeholder={f.searchablePlaceholder ?? "Nessuna (radice)"}
                                            searchPlaceholder={f.searchableSearchPlaceholder ?? "Cerca…"}
                                        />
                                    ) : (
                                        <input
