...
function ProductWorkspaceInner() {
    ...
    const tabBar = useMemo(
        () => (
            <div className="shrink-0 border-b border-slate-200 bg-white px-2 sm:px-4 pt-2 pb-1.5">
                <div className="flex max-w-6xl mx-auto gap-0.5 p-0.5 bg-slate-100/80 rounded-xl border border-slate-200/60">
                    <button
                        type="button"
                        onClick={() => setTab("library")}
                        className={
                            "flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all " +
                            (tab === "library"
