...
import { Plus, RefreshCw, Search, Layers, PlayCircle, Globe2, Trash2, Box } from "lucide-react";
...
export default function ScrapingPage() {
    ...
    const handleDeleteProject = async (projectId: number) => {
        if (!(await appConfirm("Eliminare questo progetto e tutti gli spider/job collegati?"))) return;
        try {
    ...
    const handleDeleteSpider = async (spiderId: number) => {
        if (!(await appConfirm("Eliminare questo spider e i job collegati?"))) return;
        try {
    ...
    const handleDeleteJob = async (jobId: number) => {
        if (!(await appConfirm("Eliminare questo job di scraping?"))) return;
        try {
