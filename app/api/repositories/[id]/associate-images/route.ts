...
export async function POST(
    ...
    try {
        ...
        const rawSkip = body?.productSkip;
        const rawTake = body?.productTake;
        const batchProductTake =
            typeof rawTake === "number" && Number.isFinite(rawTake) && rawTake > 0
                ? Math.min(Math.floor(rawTake), 500)
                : null;
        const batchProductSkip =
            typeof rawSkip === "number" && Number.isFinite(rawSkip) && rawSkip >= 0
                ? Math.floor(rawSkip)
                : 0;
        const useProductBatching = batchProductTake !== null;
        ...
        // 2. Prodotti staging (tutti oppure slice per batch → evita 504 su cataloghi grandi)
        let totalStagingForBatch = 0;
        const products = useProductBatching
            ? await (async () => {
                  totalStagingForBatch = await prisma.stagingProduct.count({ where: { catalogId } });
                  return prisma.stagingProduct.findMany({
                      where: { catalogId },
                      skip: batchProductSkip,
                      take: batchProductTake!,
                      orderBy: { id: "asc" },
                      select: {
                          id: true,
                          sku: true,
            ...
            : await prisma.stagingProduct.findMany({
                  where: { catalogId },
                  select: {
                      id: true,
                      sku: true,
        ...
        const nextSkip = batchProductSkip + products.length;
        const totalStaging = useProductBatching ? totalStagingForBatch : products.length;
        const hasMore = useProductBatching && nextSkip < totalStaging;
        ...
        return NextResponse.json({
            ...
            ...(useProductBatching
                ? {
                      batch: {
                          productSkip: batchProductSkip,
                          productTake: batchProductTake,
                          totalStaging,
                          processedStaging: products.length,
                          nextSkip,
                          hasMore,
                      },
                  }
