...
export async function POST(
    ...
    try {
        ...
        // 2. Read PDF File
        const fullPath = path.join(process.cwd(), "public", catalogPdf.filePath);
        if (!fs.existsSync(fullPath)) {
            return NextResponse.json({ error: "File fisico PDF non trovato." }, { status: 404 });
        }
  ...
  "products": [
    {
      ...
      "price": 0.00, // numero decimale se presente, altrimenti null
