...
export async function POST(
    ...
    try {
        ...
        const stagingRow = await prisma.stagingProduct.findFirst({
            where: { id: parsedStagingId, catalogId },
            select: { id: true, sku: true },
        });
        if (!stagingRow) {
            return NextResponse.json({ error: "Prodotto staging non trovato" }, { status: 404 });
        }
        ...
        if (String(stagingRow.sku).trim() !== String(sku).trim()) {
            return NextResponse.json({ error: "SKU non coerente con il prodotto staging" }, { status: 400 });
        }
