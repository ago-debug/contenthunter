...
export async function POST(req: Request) {
    ...
    try {
        ...
        const {
            ...
            /** Se true, importa anche prodotti “imperfetti” (es. nome mancante / prezzo non valido) usando valori null. */
            importWithErrors?: boolean;
            /** @deprecated Usa `generateSkuForMissingChannelSku` (stesso effetto). */
        ...
        for (let page = 1; processed < effectiveLimit; page++) {
            ...
            for (const wp of wooProducts) {
                ...
                const wooIdNum = integrationImportNumericId(wp?.id);
                let sku = (wp?.sku || "").toString().trim();
                let skuGeneratedFromWooId = false;
                if (!sku) {
                    if (!generateSkuForMissing || wooIdNum == null) {
                        skipped++;
                        if (reportRows.length < 250) {
                            reportRows.push({
                                sku: "",
                                wooId: wooIdNum,
                                action: "skipped",
                                message: generateSkuForMissing
                                    ? "SKU mancante e ID Woo non valido (non importabile)."
                                    : "SKU mancante (prodotto Woo non importabile).",
                            });
                        }
                        continue;
                    }
                    sku = placeholderSkuWooCommerce(wooIdNum);
                    skuGeneratedFromWooId = true;
                }
                ...
                // Resolve brand/category ids (auto-create). Usa il nome in anagrafica per il campo testo prodotto (allineato al filtro Iris).
                ...
                if (brandName) {
                    ...
                    if (dbBrand) {
                        ...
                        resolvedBrandName = dbBrand.name;
                    } else {
                        ...
                        resolvedBrandName = createdBrand.name;
                    }
                }
                ...
                try {
                    ...
                    if (!existing) {
                        const createdProduct = await prisma.product.create({
                            data: {
                                ...
                                sku,
                                brand: baseData.brand ?? displayBrand,
                    ...
                    // Validazioni “morbide”: se non importWithErrors, salta il prodotto per evitare record incompleti.
                    const softErrors: string[] = [];
                    if (!title.trim()) softErrors.push("Nome/titolo Woo mancante.");
                    if (wp?.regular_price != null && priceNum == null) softErrors.push("Prezzo non valido (regular_price).");
                    if (!importWithErrors && softErrors.length > 0) {
                        skipped++;
                        if (reportRows.length < 250) {
                            reportRows.push({
                                sku,
                                wooId: typeof wp?.id === "number" ? wp.id : null,
                                action: "skipped",
                    ...
                    if (reportRows.length < 250) {
                        const genNote = skuGeneratedFromWooId
                            ? `SKU provvisorio (assente su Woo): ${sku} — modificabile in Iris.`
                            : "";
                        const softNote = softErrors.length ? `Import con avvisi: ${softErrors.join(" ")}` : "";
                        const message = [genNote, softNote].filter(Boolean).join(" ") || undefined;
                        reportRows.push({
                            sku,
                            wooId: typeof wp?.id === "number" ? wp.id : null,
                    ...
                    if (reportRows.length < 250) {
                        reportRows.push({
                            ...
                            wooId: typeof wp?.id === "number" ? wp.id : null,
                            action: "error",
                            message: rowErr?.message ? String(rowErr.message) : "Errore sconosciuto",
                        });
                    }
                }
            }
        ...
        return NextResponse.json({
            ...
            report: {
                ...
                importWithErrors: !!importWithErrors,
                generateSkuForMissingChannelSku: !!generateSkuForMissing,
                generateSkuForMissingWooSku: !!generateSkuForMissing,
                rows: reportRows,
            },
        });
    } catch (err: any) {
