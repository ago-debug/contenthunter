...
export default function CataloguesPage() {
    ...
    /** Stessa logica delle altre pagine: utente aziendale o azienda scelta (admin globale). */
    const effectiveCompanyId =
        (session?.user as { companyId?: number | null } | undefined)?.companyId ??
        companyContext?.selectedCompanyId ??
        null;
    const companyAxiosConfig = useMemo(
        () =>
    ...
    const fetchCatalogues = async () => {
        try {
            const resp = await axios.get("/api/catalogues", companyAxiosConfig);
    ...
    const fetchBrands = async () => {
        try {
            const r = await axios.get("/api/brands", companyAxiosConfig);
        ...
        (async () => {
            try {
                const [catRes, brandsRes] = await Promise.all([
                    axios.get("/api/catalogues", companyAxiosConfig),
                    axios.get("/api/brands", companyAxiosConfig),
                ]);
                if (cancelled) return;
    ...
    }, [effectiveCompanyId, companyAxiosConfig]);
    ...
    const openAnteprime = async (c: Catalogue) => {
        ...
        setAnteprimeZoomUrl(null);
        try {
            const r = await axios.get(`/api/catalogues/${c.id}/pages`, companyAxiosConfig);
    ...
    const handleDelete = async (id: number) => {
        if (!(await appConfirm("Are you sure you want to purge this asset?"))) return;
        try {
            await axios.delete(`/api/catalogues/${id}`, companyAxiosConfig);
    ...
    const handleSaveSettings = async () => {
        ...
        try {
            await axios.patch(
                `/api/catalogues/${settingsCatalog.id}`,
                {
                            ...
                            <div className="p-10 space-y-8">
                                ...
                                <div className="grid grid-cols-1 gap-6">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <p className="text-[11px] text-slate-500 font-medium">Collega il progetto a un marchio: i prodotti inviati a Iris saranno associati a questo brand.</p>
                                        <select
                            ...
                            <div className="main-card p-8 h-full flex flex-col group hover:border-[#E6D3C1] transition-all duration-300">
                                ...
                                <div className="mt-auto pt-6 border-t border-gray-50 flex flex-wrap items-center justify-between gap-2">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <Link
                                            href={`/?tab=import&id=${catalogue.id}`}
