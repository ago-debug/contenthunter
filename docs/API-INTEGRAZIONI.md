...
| `/api/company/integration-settings` | Impostazioni integrazione (WooCommerce, PrestaShop, chiavi AI lato server) |
| `/api/integrations/woocommerce/*` | Flussi WooCommerce |
| `/api/integrations/prestashop/*` | Flussi PrestaShop 9+ (webservice: prodotti, stock, immagini, multilingua) |
...
## 7. Integrazioni e‑commerce (WooCommerce, PrestaShop)

Esistono route dedicate sotto `/api/integrations/woocommerce` e `/api/integrations/prestashop` (push e import). Per configurazione store e credenziali usare l’interfaccia **Impostazioni integrazione** per azienda e le API che leggono/aggiornano `Company` (dominio, consumer key/secret, URL/chiave webservice Presta, ecc.).

### 7.1 PrestaShop 9+ (webservice)

- **`GET /api/integrations/prestashop`** — query: `shopUrl`, `apiKey`, opz. `idShop`. Test connessione, anteprima campi e lingue attive.
- **`POST /api/integrations/prestashop`** — body JSON: `shopUrl`, `apiKey`, `product` (oggetto prodotto ERP), `mapping` (categoria default, lingua, `idTaxRulesGroup`, `idShop` multistore, `syncManufacturer`, `uploadImages`, `erpPriceIncludesVat`, stock…). Opzionale **`overwrite`**: oggetto con chiavi booleane (default tutte `true`). Se una chiave è **`false`**, in **aggiornamento** di un prodotto già presente su PrestaShop quel campo non viene sostituito con i dati ERP e resta il valore sul negozio. Chiavi supportate: `title`, `description`, `shortDescription`, `price`, `ean`, `category`, `manufacturer`, `images`, `stock`. Tipi e normalizzazione in `lib/channel-push-overwrite.ts` (`PrestaPushFieldOverwrite`). **Prezzo:** il webservice invia `<price>` come **imponibile**; con `erpPriceIncludesVat` true (default) il listino ERP viene scorporato usando l’aliquota del codice IVA del prodotto, altrimenti la prima aliquota letta dal gruppo tasse Presta (`idTaxRulesGroup`), così la colonna IVATO in BO resta coerente col listino lordo ERP. Crea/aggiorna prodotto per **reference = SKU**, testi per tutte le lingue attive in PS (da `product.translations`), produttore da `product.brand`, immagini da `product.images`, stock su `stock_availables`.
- **`POST /api/integrations/prestashop/import`** — richiede sessione e `x-company-id` (admin globale). Import in ERP: multilingua in `ProductText`, brand da manufacturer, prezzo, categorie, EAN, immagine principale (URL pubblico `img/p/…`), extra `prestashopProductId`.
- **`POST /api/integrations/prestashop/sync-images`** — body: `shopUrl`, `apiKey`, `product` (`sku`, `images`, opz. `extraFields.prestashopProductId`), opz. `mapping.idShop`, `mapping.maxImages` (default 12), `publicOrigin`. Elenca immagini prodotto su Presta, rimuove duplicati con stesso hash binario, carica da URL ERP solo file non già presenti (rispetto `maxImages`).

### 7.2 WooCommerce (REST API)

- **`GET /api/integrations/woocommerce`** — query: `domain`, `key`, `secret`. Test connessione e anteprima campi campione.
- **`POST /api/integrations/woocommerce`** — body JSON: `domain`, `key`, `secret`, `product`, `mapping` (nomi attributi brand/materiale/dimensioni, categorie default, stock da extra, ACF…). Opzionale **`overwrite`**: stessa logica di Presta (solo significativo in **aggiornamento** per SKU già presente). Chiavi: `title`, `description`, `shortDescription`, `price`, `images`, `categories`, `brand`, `stock`, `attributesExtra`, `acfMeta`. Tipi in `lib/channel-push-overwrite.ts` (`WooPushFieldOverwrite`).
- **`POST /api/integrations/woocommerce/import`** — import/sync da Woo verso ERP (vedi body nella route).
- **`POST /api/integrations/woocommerce/sync-images`** — body: `domain`, `key`, `secret`, `product` (almeno `sku` e `images[]` con `url`), opz. `publicOrigin` (per URL relativi ERP), `maxGalleryImages` (default 30). Sul prodotto Woo esistente (SKU): deduplica immagini (stesso `src` normalizzato + stesso contenuto via hash SHA-256), poi aggiunge dall’ERP le immagini mancanti. Risposta: `duplicateSrcRemoved`, `duplicateByContentRemoved`, `imagesAdded`, `imageCount`.

Credenziali e default (URL, chiave, ID categoria/lingua/negozio/tasse) sono anche su **`Company`** e in **Impostazioni integrazione**.
...
| `/api/integrations/woocommerce/sync-images` | POST |
| `/api/integrations/prestashop` | GET, POST |
| `/api/integrations/prestashop/sync-images` | POST |
| `/api/integrations/prestashop/import` | POST |
