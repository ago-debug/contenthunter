...
export default withAuth({
    ...
    callbacks: {
        authorized: ({ req, token }) => {
            ...
            if (req.nextUrl.pathname.startsWith("/api/product-images")) return true;
            if (req.nextUrl.pathname.startsWith("/uploads/")) return true;
            // Widget Personal Shopper (iframe WooCommerce) + API senza sessione
...
export const config = {
    matcher: [
         ...
         */
        "/((?!api/register|api/auth|api/fix-auth|api/storage|api/product-images|api/public|api/debug-db|login|register|embed|_next/static|_next/image|favicon.ico|uploads/).*)",
    ],
};
