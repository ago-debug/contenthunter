...
def setup_styles():
    ...
    ui.add_head_html('''
        <style>
            ...
            .group-label { font-size: 10px; font-weight: 800; color: #94A3B8; letter-spacing: 0.15em; padding: 24px 12px 12px 12px; }
...
class PIMApp:
    ...
    async def navigate_to(self, page_name: str):
        ...
        self.loading = True
        sidebar_area.refresh() # Aggiorna lo stato "active" del menu
...
async def content_area():
    ...
    if page == 'dashboard':
        with ui.row().classes('w-full justify-between items-center mb-10'):
            ...
            ui.button('NUOVO CATALOGO', icon='add').classes('rounded-xl px-8 py-6 bg-slate-900 text-white font-black text-xs shadow-xl tracking-widest').props('no-caps')
        
        with ui.grid(columns=4).classes('w-full gap-8'):
            if app_logic.data:
                for c in app_logic.data:
                    with ui.card().classes('main-card p-8 gap-5 hover:scale-[1.02] transition-transform cursor-pointer'):
                        with ui.row().classes('w-full justify-between items-start'):
                            ui.avatar('folder', color='blue-50', text_color='blue-600').classes('rounded-2xl')
                            ui.badge(c.get('status', 'STAGING').upper()).props('outline px-2 color=blue-4')
                        ui.label(c['name']).classes('text-xl font-black text-slate-800 tracking-tight')
                        ui.label(f"Importazione: {c.get('product_count', 0)} prodotti").classes('text-xs text-slate-400 font-bold')
                        ui.button('GESTISCI CATALOGO', color='primary').classes('w-full rounded-2xl mt-4 font-black py-4 text-xs').props('no-caps')
            else:
                ui.label('Nessun catalogo attivo.').classes('text-slate-400 italic')

    elif page == 'products':
        ui.label('Master ERP Inventory').classes('text-3xl font-black text-slate-900 tracking-tight mb-8')
        columns = [
            {'name': 'sku', 'label': 'SKU CODE', 'field': 'sku', 'align': 'left'},
            {'name': 'title', 'label': 'PRODUCT NAME', 'field': 'title', 'align': 'left'},
            {'name': 'brand', 'label': 'BRAND', 'field': 'brand', 'align': 'left'},
            {'name': 'price', 'label': 'MSRP PRICE', 'field': 'price', 'align': 'right'},
        ]
        ui.table(columns=columns, rows=app_logic.data, row_key='sku').classes('w-full main-card bg-white p-6')
...
async def main_page():
    ...
    # HEADER
    with ui.header().classes('bg-white/90 backdrop-blur-xl text-slate-800 border-b border-slate-200 px-10 py-4 flex justify-between items-center z-10 shadow-sm'):
        ui.label('INDUSTRIAL DATA MANAGEMENT ENGINE').classes('text-[9px] font-black opacity-30 uppercase tracking-[0.3em]')
        with ui.row().classes('items-center gap-6'):
            ui.icon('search').classes('text-slate-400 cursor-pointer')
            ui.avatar('AG').classes('text-[10px] bg-slate-900 text-white font-black shadow-lg border-2 border-slate-100')

    # SIDEBAR
    with ui.left_drawer(value=True).classes('p-6 flex flex-col gap-0 shadow-lg'):
        with ui.row().classes('items-center gap-4 mb-10 px-2'):
            with ui.element('div').classes('w-11 h-11 rounded-2xl bg-slate-900 flex items-center justify-center font-black text-white shadow-xl'):
                ui.label('CH').classes('text-sm tracking-widest')
        ...
        ui.space()
        # AI Card
        with ui.card().classes('bg-slate-900 p-5 rounded-[24px] mb-4 cursor-pointer hover:bg-slate-800 transition-all shadow-xl'):
            with ui.row().classes('items-center gap-4'):
                ui.avatar('memory', color='white', text_color='slate-900').classes('rounded-xl shadow-lg shadow-blue-900/40')
                with ui.column().classes('gap-0'):
                    ui.label('AI Hub').classes('text-[12px] font-black text-white')
...
ui.run(title=TITLE, port=3001, dark=False)
