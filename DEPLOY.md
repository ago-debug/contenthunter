...
# (binari nativi e versioni OS non coincidono).
rm -rf node_modules
npm ci
...
## 8. «Non parte nulla» / Passenger esce subito

Esegui **nella root dell’app** (dove c’è `package.json`):
...
Lo script controlla Node, `.next/BUILD_ID`, Prisma client e lancia un `next.prepare()` (senza mettere in ascolto il server): l’**ultimo errore stampato** è di solito quello che Passenger vede.

Poi prova in SSH (solo test; Ctrl+C per terminare):
...
Se qui compare un errore, è quello da correggere (build mancante, permessi, modulo mancante, ecc.). `next start` e `app.js` sono configurati per ascoltare su **`0.0.0.0`** (compatibile Passenger).

**In Plesk → Node.js:** *Application startup file* può essere `package.json` (script `start`) oppure **`app.js`** — non un path che non esiste.

## 9. Riepilogo checklist
